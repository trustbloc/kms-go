...
func Test_CreateKID(t *testing.T) {
	t.Run("ED25519 KID", func(t *testing.T) {
		...
		t.Run("KID for invalid keys", func(t *testing.T) {
			...
			require.EqualError(t, err, "createKID: failed to build jwk: invalid character 'b' looking for "+
				"beginning of value")
		})
	})
...
func TestCreateKID(t *testing.T) {
	...
	require.EqualError(t, err,
		"createKID: failed to build jwk: failed to parse ecdsa key in DER format: asn1: structure error: tags don't "+
	...
	require.EqualError(t, err, "createKID: failed to build jwk: create JWK: go-jose/go-jose: "+
		"invalid EC key (nil, or X/Y missing)")
...
func TestBuildJWKX25519(t *testing.T) {
	...
	t.Run("success buildJWK for X25519", func(t *testing.T) {
		_, err = BuildJWK(x25519, kms.X25519ECDHKWType)
		require.NoError(t, err)
	})
